...
import { Newspaper, ShoppingCart, Pencil, Trash2, Check, X, CalendarDays, Package, Hash } from 'lucide-react';

const GRAD = 'linear-gradient(135deg, #2DD4BF 0%, #3D52A0 100%)';
...
function Magazine({ id, name, price, copies, orderQty, currentIssue, onDelete, onUpdate, canManage = false, onAddToCart }) {
    ...
    const handleSave = () => {
        if (!canManage) return;
        onUpdate(id, { id, name: tempName, price: parseFloat(tempPrice), copies: parseInt(tempCopies), orderQty: parseInt(tempOrderQty), currentIssue: tempCurrentIssue ? new Date(tempCurrentIssue).toISOString() : null });
        setIsEditing(false);
    };
        ...
        return (
            <div className="edit-card">
                <div style={{ display: 'flex', alignItems: 'center', gap: 8, marginBottom: 4 }}>
                    <Newspaper size={16} color="#2DD4BF" />
    ...
    return (
        <div className="p-card">
            <div className="p-cover" style={{ background: GRAD }}>
                <Newspaper size={40} color="rgba(255,255,255,0.9)" />
